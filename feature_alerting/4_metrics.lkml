...
include: "/models/content_integration_base.model.lkml"
...
view: alerting_metrics {
  ...
  derived_table: {
    sql:
      SELECT *
      FROM
      {% if alerting_parameters.metric_name._parameter_value == 'returned_requests_rate' %}
       ${returned_requests_rate.SQL_TABLE_NAME}
      {% else %}
        SELECT 
          CURRENT_TIMESTAMP() AS time,
          NULL AS returned_requests_rate,
          NULL AS content_source,
          NULL AS office_id,
          NULL AS search_source,
          NULL AS search_engine,
          NULL AS site_currency,
          NULL AS returned_packages_count,
          NULL AS all_requests_count
        WHERE 1=0
      {% endif %} view
      {% if alerting_parameters.metric_name._parameter_value == 'returned_requests_rate' %}
      WHERE extract(time FROM view.time) <= TIME_ADD(extract(time FROM CURRENT_TIMESTAMP()), INTERVAL 15 minute)
      {% endif %};;   #  filters the data for each day to match the time period
  }
  
  # Dimensions must be explicitly defined here so they flow through to alerting_dt
  dimension: content_source {
    type: string
    sql: ${TABLE}.content_source ;;
    group_label: "2. Content"
